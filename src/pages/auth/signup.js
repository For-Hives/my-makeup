...
const schema = zod
	.object({
		...
		name: zod.string({ required_error: 'Le nom de compte est requis' }).max(255, 'Le nom de compte est trop long'),
	})
		...
		<>
			...
			<div className="relative flex h-[92vh] max-h-screen overflow-hidden md:h-screen md:overflow-auto md:bg-white">
					...
					<div className="mx-auto w-full max-w-sm lg:w-96">
										...
										<div className="mt-4 grid grid-cols-1 gap-4">
											<div className={'flex w-full justify-center'}>
												...
												>
													...
													<p className={'flex flex-nowrap font-[roboto] text-[14px] font-medium text-black/[54%]'}>{"S'inscrire avec Facebook"}</p>
												</button>
											</div>
											...
											<div className={'flex w-full justify-center'}>
												...
												>
													...
													<p className={'flex flex-nowrap font-[roboto] text-[14px] font-medium text-black/[54%]'}>{"S'inscrire avec Google"}</p>
												</button>
											</div>
										</div>
									</div>
								...
								<div className="mt-6">
									<form onSubmit={handleSubmit(onSubmit)} method="POST" className="space-y-6">
										...
										<div className={'flex items-center justify-center '}>
											Déjà un compte ?&nbsp;
											<Link className={'font-semibold text-indigo-700 hover:text-indigo-700 hover:underline'} href={'/auth/signin'}>
												Connecte toi
											</Link>
										</div>
									</form>
						...
						{!!(session && session.user && !_.isEmpty(session.user)) && (
							<div className={'mt-8'}>
								...
								<Link
									href={'/auth/profil'}
									className={'radius-2xl my-8 border-2 border-indigo-700 px-4 py-2 text-indigo-700 hover:text-indigo-700 hover:underline'}
								>
				...
				<div className="relative hidden w-full flex-1 lg:block lg:object-contain">
					...
					<Image alt={'background my-makeup'} fill src="/assets/bg_makeup.webp" className={'z-10 -scale-x-100 transform object-cover'}></Image>
				</div>
			</div>
		</>
	)
