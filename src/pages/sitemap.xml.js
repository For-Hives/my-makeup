...
export const getServerSideProps = async ({ res }) => {
	...
	let staticPaths = files
		.filter(staticPagePath => {
			...
			return !['api', 'api/sendMail', '_app.js', '_document.js', '404.js', 'sitemap.xml.js', 'index.js'].includes(base)
		})
		.map(staticPagePath => {
			let parsedPath = path.parse(staticPagePath)
			let newPath = `${parsedPath.dir}/${parsedPath.name}`.replace('src\\pages\\', '').replace('src\\pages', '').replace('src/pages', '')
			return `${process.env.NEXT_PUBLIC_URL}/${newPath}`.replace(`${process.env.NEXT_PUBLIC_URL}//`, `${process.env.NEXT_PUBLIC_URL}/`)
		})
	...
	const resultUsers = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/makeup-artistes`, {
		method: 'GET',
		headers: {
			// 	token
	...
	const resultTalents = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/talents`, {
		method: 'GET',
		headers: {
			// 	token
			'Content-Type': 'application/json',
	...
	const resultBlog = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/articles`, {
		method: 'GET',
		headers: {
			// 	token
			'Content-Type': 'application/json',
	...
	const allPaths = [...staticPaths, ...pathsUsers, ...pathsTalents, ...pathsBlog]
